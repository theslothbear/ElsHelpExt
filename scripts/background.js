...
const allResourceTypes = Object.values(browser.declarativeNetRequest.ResourceType);
...
const rules = [
  {
    ...
    condition: {
      ...
      resourceTypes: ['main_frame', 'sub_frame', 'script', 'image', 'xmlhttprequest'], // Укажите нужные типы ресурсов
    }
  }
];

browser.declarativeNetRequest.updateDynamicRules({
  removeRuleIds: [1], // удаляем существующее правило с ID '1'
  addRules: rules
}).catch(error => {
...
try{
	...
	browser.storage.local.get(['login', 'password'], async function(data) {
		...
		.then(async t => {
		    ...
		    for (var i = 1; i < 100; i++) {
		        ...
		        if (s1.length > 1) {
		            ...
		            if(i == 1){
						await browser.storage.local.get(['num'], async function(data) {
							var num = 0;
							if (typeof data.num !== 'undefined'){
								num = data.num;
							}
							await browser.storage.local.set({'num': num+1}, async function(data){
								await browser.notifications.create(`${num}`, {
									type: 'basic',
	        ...
	        await browser.storage.local.get(['marks'], async function(data) {
	        	var sp = data.marks;
	        								...
	        								for (let k=0; k<count_after-count_before; k++){
	        									...
	        									await browser.storage.local.get(['num'], async function(data) {
	    											...
	    											await browser.storage.local.set({'num': num+1}, async function(data){
			        									...
			        									await browser.storage.local.get(['history'], async function(data) {
			        										var history = [];
				        									...
				        									await browser.storage.local.set({'history': history}, async function(data){
				        										await browser.notifications.create(`${num}`, {
																	type: 'basic',
																	iconUrl: '../images/icon128.png',
																	title: 'Новая оценка',
	        	...
	        	await browser.storage.local.set({'marks': spg});
	        });
